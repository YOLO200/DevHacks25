...
type ActiveView = 'dashboard' | 'caregivers' | 'reports' | 'recordings' | 'patients' | 'care-plans' | 'settings'
...
export default function DashboardPage() {
  ...
  const fetchMedicalReports = useCallback(async () => {
    try {
      ...
      if (user) {
        ...
        if (data && data.length > 0) {
          console.log('🔍 [UI] Medical reports data preview:', data.map(r => ({
            id: r.id,
            status: r.status,
      ...
      console.error('Error fetching medical reports:', error.message || error, error)
  ...
  const deleteReport = async (reportId: string, reportTitle: string) => {
    if (!confirm(`Are you sure you want to delete the medical report for "${reportTitle}"?`)) return
  ...
  const downloadMedicalReport = (report: any) => {
    ...
    if (report.chief_complaint) {
      ...
      pdf.setFontSize(10)
      pdf.setFont('helvetica', 'normal')
      const chiefComplaintText = pdf.splitTextToSize(report.chief_complaint, 170)
          ...
          <div className="p-8 bg-gray-50 min-h-screen">
            <div className="max-w-6xl mx-auto">
                                  ...
                                  <div className="p-6">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-4">
                                      <h5 className="text-sm font-medium text-gray-700 mb-3">
                                        {structuredView ? 'Structured Transcription' : 'Raw Transcription'}
                                        {/* Debug info */}
                                        <span className="text-xs text-gray-500 ml-2">
                                          (Structured: {transcript.structured_transcript ? 'Available' : 'Not Available'})
                                        </span>
                                      </h5>
                                      <div 
                                        ...
                                        {structuredView && transcript.structured_transcript ? (
                                          <div 
                                            ...
                                            dangerouslySetInnerHTML={{
                                              __html: transcript.structured_transcript
                                                .replace(/\n\n/g, '<br><br>')
                                                .replace(/\n/g, '<br>')
                                                .replace(/^Doctor:/gm, '<strong style="color: #2563eb;">👨‍⚕️ Doctor:</strong>')
                                                .replace(/^Patient:/gm, '<strong style="color: #059669;">👤 Patient:</strong>')
                                      ...
                                      {process.env.NODE_ENV === 'development' && (
                                        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-xs">
                                          ...
                                          <p>Raw transcript length: {transcript.transcription_text?.length || 0}</p>
                                          <p>Structured transcript length: {transcript.structured_transcript?.length || 0}</p>
                                          <p>Current view: {structuredView ? 'Structured' : 'Unstructured'}</p>
                                          <p>Structured available: {transcript.structured_transcript ? 'Yes' : 'No'}</p>
                                        </div>
                                      )}
                                    </div>
              ...
              {activeTab === 'medical-reports' && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200">
                  <div className="p-6 border-b border-gray-200">
                    <h3 className="text-xl font-bold text-gray-900">AI Medical Reports</h3>
                    <p className="text-gray-600 mt-1">AI-generated medical reports from voice recordings</p>
                  </div>
                  ...
                  {medicalReports.length === 0 ? (
                    <div className="p-12 text-center">
                      ...
                      <h4 className="text-lg font-medium text-gray-900 mb-2">No medical reports yet</h4>
                      <p className="text-gray-600">Medical reports will be automatically generated when audio transcripts are completed</p>
                    </div>
                  ) : (
                    <div className="p-6">
                      <div className="space-y-4">
                        {medicalReports.map((report) => (
                          <div key={report.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                            {/* Collapsed View */}
                            <div className="flex items-center hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setExpandedReport(expandedReport === report.id ? null : report.id)}>
                              {/* Main Content */}
                              <div className="flex-1 p-4">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center space-x-3">
                                      ...
                                      {renamingReport === report.id ? (
                                        <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                                          <input
                                            ...
                                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                            onKeyDown={(e) => {
                                              if (e.key === 'Enter') saveReportName(report.id)
                                              if (e.key === 'Escape') cancelRenamingReport()
                                            }}
                                            autoFocus
                                          />
                                          <div className="flex space-x-2">
                                        ...
                                        <h4 className="font-medium text-gray-900">{report.recordings?.title || 'Medical Report'}</h4>
                                      )}
                                      <p className="text-sm text-gray-500">
                                        {new Date(report.created_at).toLocaleDateString('en-US', { 
                                          ...
                                          day: 'numeric',
                                          year: 'numeric'
                                        })} • {report.recordings?.duration ? Math.floor(report.recordings.duration / 60) : 0}:{report.recordings?.duration ? (report.recordings.duration % 60).toString().padStart(2, '0') : '00'}
                                      </p>
                                      <div className="flex items-center mt-1">
                                        <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                                          ...
                                          {report.status === 'pending' && '⏸️ Pending'}
                                        </span>
                              ...
                              <div className="flex items-center space-x-1 p-2">
                                ...
                                <button 
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    startRenamingReport(report.id, report.recordings?.title || 'Medical Report')
                                  }}
                                  className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
                                  title="Rename Report"
                                >
                                ...
                                <button 
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    deleteReport(report.id, report.recordings?.title || 'Medical Report')
                                  }}
                                  className="p-1.5 text-red-600 hover:bg-red-100 rounded-lg transition-colors"
                                  title="Delete Report"
                                >
                                ...
                                <div className="flex items-center text-gray-400">
                                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fillRule="evenodd" d={expandedReport === report.id ? "M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" : "M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"} clipRule="evenodd" />
                                  </svg>
                                </div>
                              </div>
                            </div>
                              ...
                              <div className="border-t border-gray-200">
                                  ...
                                  <div className="p-6">
                                      ...
                                      <div className="space-y-6">
                                        ...
                                        {report.chief_complaint && (
                                          <div className="bg-blue-50 rounded-lg p-4">
                                            <h5 className="text-sm font-semibold text-blue-800 mb-2">Chief Complaint</h5>
                                            <p className="text-blue-700">{report.chief_complaint}</p>
                                          </div>
                                        )}

                                        {/* Patient Demographics */}
                                        {report.patient_demographics && Object.keys(report.patient_demographics).length > 0 && (
                                          <div className="bg-gray-50 rounded-lg p-4">
                                            <h5 className="text-sm font-semibold text-gray-800 mb-2">Patient Demographics</h5>
                                            <div className="space-y-1 text-sm text-gray-700">
                                              {report.patient_demographics.age && <p>Age: {report.patient_demographics.age}</p>}
                                              {report.patient_demographics.gender && <p>Gender: {report.patient_demographics.gender}</p>}
                                              {report.patient_demographics.other_details && <p>{report.patient_demographics.other_details}</p>}
                                            </div>
                                          </div>
                                        )}
                                        ...
                                        {report.red_flags && report.red_flags.length > 0 && (
                                          <div className="bg-red-50 rounded-lg p-4">
                                            <h5 className="text-sm font-semibold text-red-800 mb-2">🚨 Red Flags</h5>
                                      ...
                                      <div className="space-y-6">
                                        ...
                                        {report.patient_summary && (
                                          <div className="bg-green-50 rounded-lg p-4">
                                            <h5 className="text-sm font-semibold text-green-800 mb-2">Patient Summary</h5>
                                            <p className="text-green-700 text-sm leading-relaxed">{report.patient_summary}</p>
                                          </div>
                                        )}

                                        {/* Medical History */}
                                        {report.medical_history && Object.keys(report.medical_history).length > 0 && (
                                          <div className="bg-purple-50 rounded-lg p-4">
                                            <h5 className="text-sm font-semibold text-purple-800 mb-2">Medical History</h5>
                                            <div className="space-y-2 text-sm text-purple-700">
                                              {report.medical_history.past_medical && <p><strong>Past Medical:</strong> {report.medical_history.past_medical}</p>}
                                              {report.medical_history.medications && <p><strong>Medications:</strong> {report.medical_history.medications}</p>}
                                              {report.medical_history.allergies && <p><strong>Allergies:</strong> {report.medical_history.allergies}</p>}
                                            </div>
                                          </div>
                                        )}
                                      </div>
                                    ...
                                    {report.soap_note && (
                                      <div className="mt-6 bg-gray-50 rounded-lg p-4">
                                        <h5 className="text-sm font-semibold text-gray-800 mb-3">SOAP Note</h5>
                                ...
                                {report.status === 'failed' && (
                                  <div className="p-6 text-center">
                                    <div className="text-red-600 mb-2">❌ Report generation failed</div>
                                    <p className="text-sm text-gray-600">{report.error_message || 'Unknown error occurred'}</p>
        ...
        return (
          <div className="p-8">
            <div className="max-w-6xl mx-auto">
              ...
              <p className="text-gray-600 mb-8">Manage care plans for your patients</p>
              <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-200 text-center">
                <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <span className="text-2xl">📋</span>
                </div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">Care Plans</h3>
                <p className="text-gray-600">Care plan management features coming soon.</p>
              </div>
            </div>
          </div>
        )
