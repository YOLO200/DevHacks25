...
export default function MedicalReportsView({
  ...
  const sendEmailReport = async () => {
    ...
    try {
      ...
      showNotification((error as Error).message || 'Failed to send email', 'error')
    } finally {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200">
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-gray-600 mt-1">AI-generated medical reports from your voice recordings</p>
      </div>
      ...
      {medicalReports.length === 0 ? (
        <div className="p-6">
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6">Your AI-generated medical reports will appear here after voice recordings are processed.</p>
          </div>
        </div>
      ) : (
        <div className="p-6">
          <div className="space-y-4">
            {medicalReports.map((report) => (
              <div key={report.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="flex items-center hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => toggleReport(report.id)}>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1">
                          {renamingReport === report.id ? (
                            ...
                            <h4 className="font-medium text-gray-900">{report.recordings?.title}</h4>
                          )}
                          <p className="text-sm text-gray-500">
                            {formatDate(report.created_at)}
                          </p>
                          <div className="flex items-center mt-1">
                            <span className={`px-2 py-1 text-xs rounded-full font-medium ${getReportTypeColor(report.type)}`}>
                    ...
                    <div className="space-y-6">
                      ...
                      {report.diagnosis && (
                        <div>
                          ...
                          <p className="text-gray-700 leading-relaxed">{report.diagnosis}</p>
                        </div>
                      )}

                      {report.treatment_recommendations && report.treatment_recommendations.length > 0 && (
                        <div>
                          <h5 className="font-semibold text-gray-900 mb-2">💊 Treatment Recommendations</h5>
                          <ul className="space-y-2">
                            {report.treatment_recommendations.map((rec, index) => (
                      ...
                      {report.soap_note && (
                        <div>
                          <h5 className="font-semibold text-gray-900 mb-2">📋 SOAP Note</h5>
                          <div className="bg-white p-4 rounded-lg border border-gray-200">
                            <pre className="whitespace-pre-wrap text-sm text-gray-700 font-mono">{report.soap_note}</pre>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      {showShareModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Share Medical Report</h3>
            
            {caregivers.length > 0 && (
              <div className="mb-4">
                <h4 className="font-medium mb-2">Share with Caregivers:</h4>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {caregivers
                    .filter(c => c.status === 'accepted')
                    .map((caregiver) => (
                    <label key={caregiver.id} className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        ...
                        className="rounded"
                      />
                      <span className="text-sm">{caregiver.caregiver_name} ({caregiver.caregiver_email})</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="mb-4">
              ...
              >
                {shareEmailExpanded ? '− Email Options' : '+ Email to Custom Address'}
              </button>
            </div>
